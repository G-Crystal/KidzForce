...
Qlinqo.Setup = function (playfieldLayer, foregroundLayer, backgroundLayer, debugLayer, statusLayer, gameOverLayer, jsPromise) {
	Qlinqo.jsPromise = jsPromise;
	//setup sound effects
	...
	Qlinqo.defaultFillColor = new Qlinqo.Util.ColorRGBA(100, 175, 28, 255);//(0,128,0,255); // fill color
	Qlinqo.defaultStrokeColor = new Qlinqo.Util.ColorRGBA(52, 18, 76, 255);//(0,102,34,255); // stroke color
    ...
    contactListener.BeginContact = function(contact) {
		...
		if(drawnFixedObject) {
			...
			if(!gamePieceDrawnObject.scored)
			{
				...
				Qlinqo.pointTextObject.layer.needsDisplay = false;
	...
	backgroundLayer.addChild(new Qlinqo.NeonText(scaledWidth/2, 0.4, "Tap Here", 26, "center", "#fff", "#fff"));
	...
	var pointValues = [0, 10, 20, 30, 50, 100, 50, 30, 20, 10, 0];
	for(var col = 0; col < cols + 1; col++) {
		...
		backgroundLayer.addChild(new Qlinqo.PointValueText(0.45 + (col * xDist), groundTop + 4.8, pointValues[col])); 
	}
	...
	for(var i = Qlinqo.gamePieces.length - 1; i >= 0; i--)
	{
		...
		if(!gamePiece.body.IsAwake() && gamePiece.scored)
		{
			...
			if(Qlinqo.ballsScored == 1) {
				Qlinqo.jsPromise.resolve(Qlinqo.currentPlayerPoints);
...
Qlinqo.PointValueText = Stratiscape.DrawnObject.extend({ //PointValueText drawn object class
	...
	draw: function(ctx) {
		...
		ctx.fillStyle = '#FFF';
		ctx.strokeStyle = '#EEE'
		ctx.font = "bolder 18px Roboto,Arial,sans-serif";
		ctx.textAlign = "center";
